...
?>
  ...
  <head>
    ...
    <title>Reserva de Veículos 2</title>
...
<?php  for ($s=0; $s < count($veiculos); $s++) {  ?>
<div class="Row">
        <div class="Cell"><img src="img/<?php echo $veiculos[$s];?>" class='img-pic'></div>
        <div class="Cell_chart"><div id="slider_<?php echo $s;?>"></div></div>
        <div class="Cell_btn"><span class="span-btn btn-block"><input id="<?php echo $s;?>" type="button" name="btn"  class="btn-style"  value=" " onclick='javascript: reserva("<?php echo $s;?>");'  data-toggle="modal" data-target="#ModalReservar" /></span></div>
</div><?php  }  ?>
</div>
...
<?php  for ($s=0; $s < count($veiculos); $s++) {  ?>
       ...
       setTimeout($('#slider_<?php echo $s;?>').dateRangeSlider({range:{min: {hours: 1},max: {hours: 10}}},
	{ step: { hours: 1}},
        {valueLabels: 'hide'},
        {symmetricPositionning: false},
        {arrows:false},
        {bounds: {min: new Date(<?php echo $ano;?>,<?php echo $mes;?>,<?php echo $dia;?>, 0,  0, 0), max: new Date(<?php echo $ano;?>,<?php echo $mes;?>,<?php echo $dia;?>, 10, 0, 0)},
        defaultValues: {min: new Date(<?php echo $ano;?>,<?php echo $mes;?>,<?php echo $dia;?>,<?php echo ($hora_agenda-7)?>,0,0), max: new Date(<?php echo $ano;?>,<?php echo $mes;?>,<?php echo $dia;?>,<?php echo ($hora_agenda-7)?>,59,59)},
        scales: [{
         ...
         label: function(value){ return horas[value.getHours()]+'h';},
      	format: function(tickContainer, tickStart, tickEnd){ $('#slider_<?php echo $s;?> .ui-rangeSlider-bar').attr('id','reserva-<?php echo $s;?>');}}]
  }),500);
 <?php
 }
 ?>
...
var calendarPicker1 = $("#calendario").calendarPicker({date:"<?php echo $_GET['d'] ?>", showDayArrows:false,callback:function(cal) { atualiza_data(cal.currentDate.getFullYear(),(cal.currentDate.getMonth()+1),(cal.currentDate.getDate()));}});
</script>
...
<input type="hidden" id="ultima_data" value="<?php echo $data_filtro?>">
<input type="hidden" id="data_agora">
<input type="hidden" id="mem_ids" value="<?php echo $ids;?>">
<input type="hidden" id="nome_user" value="<?php echo $_SESSION['nome_user']; ?>">
